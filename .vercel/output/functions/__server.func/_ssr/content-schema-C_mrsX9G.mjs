//#region node_modules/.nitro/vite/services/ssr/assets/content-schema-C_mrsX9G.js
var CONTENT_TYPES = [
	{
		id: "profile",
		label: "Profile",
...
function pathForType(type, slug) {
	if (type.kind === "file") return type.path;
	return `${type.path}/${slug}.md`;
}
function inferTypeIdFromPath(path) {
	const exact = CONTENT_TYPES.find((type) => type.kind === "file" && type.path === path);
	if (exact) return exact.id;
	return CONTENT_TYPES.find((type) => type.kind === "collection" && path.startsWith(`${type.path}/`))?.id;
}
function slugify(input) {
	return input.normalize("NFKD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/['’"]/g, "").replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 80);
}
...
function serializeDoc(fm, body) {
	return [
		...
		...fm.type === "research" ? [`abstract: ${escapeYaml(fm.abstract ?? "")}`] : [],
...
function parseDoc(raw, path = "", typeId) {
	const match = /^---\r?\n([\s\S]*?)\r?\n---\r?\n?([\s\S]*)$/.exec(raw);
	...
	const tags = (fm["tags"] ?? "").replace(/^\[|\]$/g, "").split(",").map((t) => unquote(t)).filter(Boolean);
	const fallbackSlug = path.split("/").pop()?.replace(/\.md$/, "") ?? "";
	const inferredType = typeId ?? inferTypeIdFromPath(path) ?? "article";
	const type = unquote(fm["type"] ?? inferredType) || inferredType;
	...
	const extra = Object.fromEntries(Object.entries(fm).filter(([key]) => !known.has(key)));
	return {
		...
		abstract: type === "research" ? unquote(fm["abstract"] ?? "") || null : null,
...
//#endregion
export { slugify as a, serializeDoc as i, parseDoc as n, typeById as o, pathForType as r, CONTENT_TYPES as t };
