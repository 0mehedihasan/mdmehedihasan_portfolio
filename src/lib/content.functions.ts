...
export const listContent = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const gh = await import("./github.server");
  if (!gh.isGitHubConfigured()) {
    return { ok: false as const, items: [] as ContentDoc[], message: "GitHub is not configured yet." };
  }
  try {
    return { ok: true as const, items: await store.loadAll(), message: "" };
  } catch (error) {
    return { ok: false as const, items: [] as ContentDoc[], message: store.toMessage(error) };
  }
});
...
export const getContent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      if (!doc) return { ok: false as const, doc: null, message: "That file was not found on GitHub (404)." };
...
export const saveContent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      return {
        ok: true as const,
        message: commit.created ? "Created and committed to GitHub." : "Updated and committed to GitHub.",
        commit,
      };
    } catch (error) {
...
export const deleteContent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      return { ok: true as const, message: "Deleted and committed to GitHub.", commit };
    } catch (error) {
      return { ok: false as const, message: store.toMessage(error), commit: null };
    }
  });
...
export const publicContent = createServerFn({ method: "GET" }).handler(async () => {
  const store = await import("./content-store.server");
  const gh = await import("./github.server");
  if (!gh.isGitHubConfigured()) return { items: [] as ContentDoc[], available: false as const };
  try {
